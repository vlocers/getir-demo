import React from "react";
import Getir from "../images/homeimg.svg";
import { HiLocationMarker } from "react-icons/hi";
import { IoIosArrowForward } from "react-icons/io";

function Home() {
  return (
    <div>
      <div
        style={{ backgroundColor: "#6943D7" }}
        className="w-full h-[500px] hidden md:flex"
      >
        <div className="container flex items-center justify-around mx-auto lg:justify-between h-[500px]">
          <div className="flex flex-col gap-y-9 md:px-10 ">
            <img src={Getir} className="text-base md:w-44 " />
            <p className="flex text-2xl font-bold text-white">
              Dakikalar içinde kapında
            </p>
          </div>

          <div className="flex flex-col items-center p-10 bg-white gap-y-4">
            <h4 className="text-xl font-semibold text-primary">
              Giriş yap veya kayıt ol
            </h4>
            <div className="flex justify-between">
              <input
                type="text"
                className="w-2/6 py-2 border px-1.5 border-gray-400 rounded-lg outline-"
                placeholder="+90"
              />
              <input
                type="text"
                placeholder="Telefon numarası"
                className="w-3/6 py-2 border px-1.5 border-gray-400 rounded-lg"
              />
            </div>

            <button className="w-full py-3 font-semibold bg-yellow-300 rounded-md text-primary">
              Telefon numarası ile devam et
            </button>
          </div>
        </div>
      </div>
      <div className="flex flex-col md:hidden">
        <div className="flex items-center justify-center w-full h-10" style={{ backgroundColor: "#6943D7" }}>
          <img src="data:image/svg+xml;base64,"></img>

        </div>
        <div className="flex justify-between py-2 bg-amber-50">
          <p className="flex items-center font-semibold text-gray-600 ">
            <HiLocationMarker className="text-3xl text-primary" /> Teslimat
            Adresini Belirle
          </p>
          <button>
            <IoIosArrowForward className="text-3xl text-primary" />
          </button>
        </div>
        <div className="flex flex-col items-center p-10 bg-white gap-y-4">
          <h4 className="text-xl font-semibold text-primary">
            Giriş yap veya kayıt ol
          </h4>
          <div className="flex justify-between">
            <input
              type="text"
              className="w-2/6 py-2 border px-1.5 border-gray-400 rounded-lg outline-"
              placeholder="+90"
            />
            <input
              type="text"
              placeholder="Telefon numarası"
              className="w-3/6 py-2 border px-1.5 border-gray-400 rounded-lg"
            />
          </div>

          <button className="w-full py-3 font-semibold bg-yellow-300 rounded-md text-primary">
            Telefon numarası ile devam et
          </button>
        </div>
      </div>
    </div>
  );
}

export default Home;
